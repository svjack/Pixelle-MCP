...
10. **Change Video Background by RMBG**:
    - 01. `change_video_background_by_image` - Image & Video change background.
      ```bash
      # 示例用法1：
    ...
    - 02. `change_video_background_by_video` - Video & Video change background.
      ```bash
      # 示例用法1：
      # 通过z_image 生成一个512x512的男青年头像 ace生成一段中文浪漫歌曲30s 
      # 再使用音频驱动float模型生成有声视频，有声视频长宽都为512
      
      # 生成一个宽832高648的81frame樱花飘落的纯背景动态视频
      
      # 使用视频背景替换工具，使用樱花背景视频替换float的视频背景 
      # 并对替换后的视频添加原视频的音频,
      # 注意在添加音频时要保持与替换背景后视频的新宽高不变。（宽832高648）
      
      # 示例用法2：
      # 使用z_image 生成一个宽480高832的星空背景正面上半身韩国帅青年头像
      # （身子和人脸必须正面镜头）
      # ace生成一段男声中文浪漫歌曲30s 
      # 使用9段数字人工具以图片和歌曲为输入生成有声视频 
      
      # 生成一个宽480高832的63frame星空下的纯背景动态视频
      
      # 使用视频背景替换工具，使用星空背景视频替换数字人的视频背景 
      # 并对替换后的视频添加原视频的音频,
      # 注意在添加音频时要保持与替换背景后视频的新宽高不变。（宽480高832）
      
      # 注意这里视频背景替换中的前景视频为数字人视频中的最长的最后一个
      ```
...
1. **Html template render to PNG**: `html_template_render` - html template (image + title + text fields) render to image, can produce short videos' frame, PPT and so on.
   ...
   - 03. ACE music -> render to Short music video with audio: (video frame):
     - 001
       ...
       # 不执行(大概的逻辑：生成音乐->asr得到音乐内容->根据带时间戳的字幕生成3个对应时间点的应景图片->html渲染3个有文字的图片
       ...
       # （注意只使用3中要求的字体），并使得图片占较大部分。html的text部分使用一些符合图片内容的浪漫语句。在合并视频时高resize到832宽到480，
       # 每段子视频没有间隔或缓冲，所有时间加起来为45s。执行到生成对应的无声视频片段，暂时不进行后续连接和加声。
       # 直接执行，不再列出更改后的计划或中间生成的html模板。
       
       # step2:exec prompt2:
       # 继续执行视频连接（高resize到832宽到480）和将生成音乐作为背景音乐。
       ```
     ...
     - 002
       ...
       # （注意只使用3中要求的字体），并使得图片占较大部分。html的text部分使用一些符合图片内容的浪漫语句。
       # 在合并视频时高resize到832宽到480，每段子视频没有间隔或缓冲，所有时间加起来为90s。
       # 直接执行，不再列出更改后的计划或中间生成的html模板。
       ```
...
2. **Html dynamic template render to Video**: `html_dynamic_template_render` - html dynamic template (support by js) (image + title + text fields) render to video, can produce short videos.
   ...
   - 02. render to 3 rotating playback discs:
     ...
     # 并生成统一的流行风格(宽480高832)html动态模板代码（圆形裁剪图片在模板中旋转，
     # 采用红蓝动态粒子闪烁效果，标题和text都有大小反复放缩效果,
     # 字体采用要符合模板说明中的要求，文字不包含换行符，模板中不包含任何按钮，且结构简单，直接使用你生成的模板，不进行打印），
     # 并进行3次渲染，每次渲染30s。3个视频连接成一个视频(最长边resize到832)，再使用ace生成对应时长的背景音乐加入到视频里面。
     # (最长边resize到832)
     
     # 示例用法2：
     # 完成一个5张法国历史博物馆的广告任务,使用z_image_turbo生成5张图片，图片中不包含任何文字，
     # 并生成统一的流行风格html动态模板代码（圆形裁剪图片在模板中浮动放缩旋转，并建立法国三色旗半透明图层进行美化，
     # 采用法国三色旗在背景飘扬的效果，标题和text都有大小反复放缩效果,
     # 字体采用要符合模板说明中的要求，文字不包含换行符，模板中不包含任何按钮，且结构简单，直接使用你生成的模板，不进行打印），
     # 并进行5次渲染，每次渲染30s。5个视频连接成一个视频，再使用ace生成对应时长的背景音乐加入到视频里面。
     ```
